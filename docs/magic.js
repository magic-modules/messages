...
var initialState = {
  ...
  'menu': [{
    ...
    'items': [{
      'text': 'custom message type',
      'to': '-custom-message-type'
    }],
    'text': 'usage',
    'to': '/#usage'
  }, {
...
var Messages = function Messages(props) {
  return div({
    "class": 'Messages'
  }, ul(props.messages.map(function (msg) {
    return li({
      "class": _defineProperty({
        Message: 1
      }, msg.type, 1)
    }, [msg.title && h3(msg.title), typeof msg.content.map === 'function' ? msg.content.map(function (a) {
      return p(a);
    }) : msg.content && p(msg.content), button({
      onclick: [actions.messages.hide, msg]
    }, 'x')]);
  })));
};
...
var actions = {
  ...
  'messages': {
    'add': function add(state, message) {
      ...
      if (message.duration) {
        return [_objectSpread({}, state), [effects.messages.wait, message]];
      }

      return _objectSpread({}, state);
    },
    'hide': function hide(state, message) {
...
var effects = {
  'messages': {
    'wait': function wait(dispatch, message) {
      if (message.duration < 60) {
        message.duration *= 1000;
      }

      setTimeout(function () {
        return dispatch([actions.messages.hide, message]);
      }, message.duration);
    }
  },
  'pages': {
...
var pages = {
  '/messages/': function messages(state) {
    ...
    return [h1(state.title), p(['this is the ', Link({
    ...
    }, '@magic-modules'), ' Messages component. ', state.description]), GitBadges('magic-modules/messages'), h2({
      id: 'installation'
    }, 'installation:'), Pre('npm install @magic-modules/messages'), h2({
      id: 'usage'
      ...
      onclick: [actions.messages.add, message]
    }, 'add message'), button({
      onclick: [actions.messages.add, error]
    }, 'add error'), button({
      onclick: [actions.messages.add, warning]
    }, 'add warning'), button({
      onclick: [actions.messages.add, success]
    }, 'add success'), button({
      onclick: [actions.messages.add, permanentMessage]
    }, 'add permanent message'), h3({
      id: 'usage-custom-message-type'
    }, 'custom messages type'), Pre("\nexport const View = () => {\n  const msg = {\n    type: 'newType',\n    title: 'newType title',\n    content: 'newType content',\n    duration: 5,\n  }\n\n  return button({ onclick: [actions.messages.add, msg] })\n}\n\n// needed in the ssr step for now,\n// gets overwritten by @magic-modules/messages with the correct actions.\nexport const actions = {\n  messages: {},\n}\n\nexport const style = vars => ({\n  '.Messages': {\n      '.Message.newType': {\n        h3: {\n          color: vars.colors.pink[900],\n        },\n      },\n    },\n  }\n})\n  "), button({
      onclick: [actions.messages.add, newType]
    }, 'add custom message'), h2({
      id: 'source'
      ...
      to: 'https://github.com/magic/core'
    }, '@magic/core')]), LightSwitch(state), Messages(state)];
  },
  '/messages/404/': function messages404() {
